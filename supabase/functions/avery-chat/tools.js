// Reusable EskoDSK AI capability layer.
// Business-specific values come from the authenticated business context and configuration,
// not from the tool names or operational query logic.
export const lookupToolDefinitions = {
  lookup_pickup: {
    name: "lookup_pickup",
...
export const allowedToolTables = new Set(Object.values(lookupToolDefinitions).map((definition) => definition.table));

export const CLIENT_PICKUP_SAFE_COLUMNS = [
  "id",
  "tracking_number",
  "pickup_address",
  "delivery_address",
...
export const OWNER_EMPLOYEE_CONTEXT = "owner_employee";

export const CLIENT_ADA_SYSTEM_PROMPT = `You are Ada, the customer-facing Virtual Dispatch Coordinator for Dasher Lab Medical Courier.
...
export const OWNER_EMPLOYEE_ADA_SYSTEM_PROMPT = `You are Ada, the authenticated owner/employee dispatch assistant for the current business.
...
export function resolveAdaContext(rawContext) {
...
export function getAllowedToolsForContext(context = CLIENT_CONTEXT) {
...
export function getLookupPickupColumnsForContext(context = CLIENT_CONTEXT) {
  const resolvedContext = resolveAdaContext(context);
...
export function extractTrackingNumberFromMessage(message = "") {
  ...
  const patterns = [
    /\b[A-Za-z]{2,}-\d{8}-\d{5}\b/g,
    /\btracking\s*(?:number|id)?\s*[:#-]?\s*([A-Za-z]{2,}-\d{8}-\d{5})\b/i,
    /\bpickup\s+(?:#|no\.?\s*)?([A-Za-z]{2,}-\d{8}-\d{5})\b/i,
    /\b([A-Za-z]{2,}-\d{8}-\d{5})\b/
  ];

  for (const pattern of patterns) {
    const match = normalized.match(pattern);
    if (match) {
      const candidate = Array.isArray(match) ? match[0] : match[1];
      if (candidate) {
        return candidate;
      }
    }
...
export function isClientOperationalQuery(message = "") {
  ...
  return operationalPatterns.some((pattern) => normalized.includes(pattern));
}

export function buildOpenAIToolDefinitionsForContext(context = CLIENT_CONTEXT) {
...
export function validateToolTableName(tableName) {
...
export function resolveOperationalWindow(windowName = "today", baseDate = new Date()) {
...
export function validateWindowValue(value) {
...
export function assertNoBusinessIdOverride(toolName, rawArgs = {}) {
...
export function applyAuthenticatedBusinessScope(toolName, filters = {}, authenticatedBusinessId) {
...
export function validateToolRequest(toolName, rawArgs = {}) {
...
export function applyOperationalDateFilters(query, filters = {}) {
